...
const SliderArrow = ({ direction, onClick }) => {
  ...
  const positionClass = isLeft
    ? 'left-[-25px] max-md:left-5'
    : 'right-[-25px] max-md:right-5';
  ...
  return (
    <button
      onClick={onClick}
      className={`flex absolute top-2/5 gap-2 justify-center items-center p-3 backdrop-blur-sm -translate-y-2/4 cursor-pointer bg-neutral-700 rounded-[50px] max-sm:hidden ${positionClass}`}
